...
(function($) {
	$(function() {
		...
		//HOMEPAGE SCROLLER
		var features = $('.index-page #masthead ul').children().length;
		$('.index-page #masthead ul').css({width:features*$(document).width()});
		if(is_mobile == true){
			$('#masthead, #masthead .children, #masthead li.child,#masthead li.child .holder,#masthead .hit,#masthead .overlay').css({width:$(document).width()});
		}
		$('.index-page #masthead').serialScroll({
			target:'.children',
			items:'.child',
			next:'.next',
			prev:'.prev',
		...
		//LIGHTSPEED ITALIZE
		$("body *").replaceText( 'LightSpeed', italize );
		//NO WIDOWS IN TITLES
	...
	function italize( str ){
		return "Light<em>Speed<\/em>";
	};
})(jQuery);
